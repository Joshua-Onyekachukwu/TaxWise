import { CsvAdapter, ColumnMapping, NormalizedTransaction } from './types.ts';
...
export class GenericAdapter implements CsvAdapter {
    ...
    async parseRow(row: any, mapping: ColumnMapping): Promise<NormalizedTransaction> {
        ...
        tx.fingerprint = this.generateFingerprint(tx);

        return tx;
    }

    private generateFingerprint(tx: NormalizedTransaction): string {
        const key = `${tx.date}-${tx.amount.toFixed(2)}-${tx.description.substring(0, 50)}`;
