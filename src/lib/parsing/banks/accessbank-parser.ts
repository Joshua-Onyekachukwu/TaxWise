...
export class AccessBankParser implements BankStatementParser {
  bankIdentifier = 'accessbank';

  compatibilityTest = /Access Bank/i;

  parse(text: string): NormalizedTransaction[] {
    const transactions: NormalizedTransaction[] = [];
    const statementBody = text.substring(text.indexOf('Date'), text.lastIndexOf('Total'));
    const lines = statementBody.split('\n').filter(line => line.trim() !== '');

    const transactionRegex = /^(?<date>\d{2}\/\d{2}\/\d{4})\s+(?<description>.+?)(?=\s{2,}|$)/;
